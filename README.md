...
"dg_fcmri_wl_wholeGroup.m" runs the same processing and 1st-level analysis but calculates FC within phonological or visual conditions from a replication of the language paradigm of Ebner et al. (2011) [https://doi.org/10.1016/j.neuroimage.2011.06.048]. As a result, it is possible to run with the GUI a 2nd-level analysis to compare FC between patients and controls during phonological processing or visual pattern discrimination. The script includes a mask to calculate FC only between ROIs relevant for phonological awareness and reading, downloaded from Neurosynth: (https://neurosynth.org/analyses/terms/phonological/) AND (https://neurosynth.org/analyses/terms/reading/).

"dg_fcmri_wl_wholeGroup.m" runs the same processing and 1st-level analysis but calculates FC within encoding or retrieval conditions from a replication of the verbal working memory paradigm of Siffredi et al. (2017) [https://doi.org/10.1371/journal.pone.0179959]. As a result, it is possible to run with the GUI a 2nd-level analysis to compare FC between patients and controls during encoding or retrieval. The script includes a mask to calculate FC only between ROIs relevant for verbal working memory, downloaded from Neurosynth:(https://neurosynth.org/analyses/terms/verbal%20working/).
